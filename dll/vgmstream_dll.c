...
#include "../vgmstream/src/util/samples_ops.h"
...
static int fill_dll_config(cli_config* cfg, const char* filename, const char* outname ) {
    int opt;
    ...
    cfg->infilename = filename;
    cfg->outfilename = outname;
...
static STREAMFILE* open_streamfile_buffer(const void* infile, const size_t buf_size, const char* const filename);
...
static STREAMFILE* stdio_open(STREAMFILE* sf, const char* const filename, size_t buf_size) {
    if (!filename)
        return NULL;
    ...
    uint8_t* buf = NULL;
    int ret = 0;
    buf = calloc(sf->get_size(sf), sizeof(uint8_t));
    if (!buf) return NULL;
    ret = sf->read(sf, buf, 0, sf->get_size(sf));
    //memcpy(buf,sf->buf, sf->buf_size);
    return open_streamfile_buffer(buf,  sf->get_size(sf), filename);
}
...
static STREAMFILE* open_streamfile_buffer(const void* infile, const size_t buf_size, const char* filename) {
    uint8_t* buf = NULL;
    ...
    memcpy(this_sf->buf, infile, this_sf->buf_size);
    this_sf->buf_offset = 0;
    this_sf->valid_size = buf_size;
...
int convert_lwav(int argc, char** argv) {
    cli_config cfg = {0};
    int i, res, ok;
...
__declspec(dllexport) int __cdecl convert_buf(const void* buf , const size_t buf_size, const char* fname, const char* outname, const bool verbose) {
    cli_config cfg1 = {0};


    int res;
    res = fill_dll_config(&cfg1, fname, outname);
    if (!res) goto fail;
    cli_config* cfg = &cfg1;
    ...
    /* open streamfile and pass subsong */
    {
        ...
        STREAMFILE* sf = open_streamfile_buffer(buf, buf_size, fname);
        if (!sf) {
